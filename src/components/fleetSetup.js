...
function addHighlightCells(x, y, length, orientation) {
  ...
  if (orientation) { // Vertical placement
    ...
    if(i<length){
      for(let j=0;j<i;j++) document.querySelectorAll('.cell')[(x + j) * 10 + y].classList.add('invalid');
    }
    else if(invalid){
      for(let j=0;j<length;j++) document.querySelectorAll('.cell')[(x + j) * 10 + y].classList.add('invalid');
    }
    ...
    if(i<length){
      ...
      for(let j=0;j<length;j++) document.querySelectorAll('.cell')[x * 10 + y+j].classList.add('invalid');
    }
  }
}
          ...
          if(!targetCell.classList.contains('invalid')){
            ...
            if(orientation){
              for(let i=0;i<length;i++){
                if(document.querySelectorAll('.cell')[(startCoords[0]+i)*10+startCoords[1]].classList.contains('invalid')){ 
                  valid=false;
                  break;
                }
              }
              ...
              for(let i=0;i<length;i++){
                if(document.querySelectorAll('.cell')[startCoords[0]*10+startCoords[1]+i].classList.contains('invalid')){ 
                  valid=false;
                  break;
                }
              }
...
function placeShipUI(name,startCoords,orientation) {
  ...
  if(name=='submarine') shipImg.style.width=`calc(var(--cell-size)*${length}*0.94)`
  if (orientation) {
